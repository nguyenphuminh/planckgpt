...
class MuonCompiled(Muon):
    """
    Compiled version of Muon with fused kernel for maximum speed.
    Trades flexibility for performance.
    """
    
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Compile the core update function
    ...
    @torch.no_grad()
    def step(self, closure=None):
        """Perform a single optimization step using compiled kernel."""
        loss = None
        if closure is not None:
            with torch.enable_grad():
                loss = closure()
        ...
        for group in self.param_groups:
            ...
            for param in params_2d:
                ...
                # State initialization (same as parent)
                if len(state) == 0:
                    state["step"] = 0
                    state["momentum_buffer"] = torch.zeros_like(grad)
                    if grad.size(-2) <= grad.size(-1):
    ...
    def _fused_step(
        ...
        """Fused step - all operations in one compiled graph."""
        # Nesterov momentum
        momentum_buffer.lerp_(grad, 1 - momentum)
        g = grad.lerp_(momentum_buffer, momentum)
        ...
        if wd > 0:
            mask = (g * param) >= 0
            param.sub_(lr * g + lr * wd * param * mask)
        else:
            param.sub_(lr * g)
